 ...
 */
struct capability_info primary_processor_based[21] =
{
 ...
 * See SDM volume 3C, section 24.6.2
 */
struct capability_info secondary_processor_based[27] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Primary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(primary_processor_based, 21, lo, hi);

	/* Secondary Processor based controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Secondary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secondary_processor_based, 27, lo, hi);
